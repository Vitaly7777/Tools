import sys
from PyQt5.QtWidgets import QApplication, QMainWindow, QRadioButton, QLineEdit, QPushButton, QLabel, QVBoxLayout, QHBoxLayout, QWidget, QMessageBox
...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        button_layout = QHBoxLayout()  # Создаем горизонтальный layout для кнопок
        self.button_convert = QPushButton("Преобразовать")
        self.button_convert.clicked.connect(self.on_convert_clicked)
        button_layout.addWidget(self.button_convert)  # Добавляем кнопку в горизонтальный layout
        
        self.button_copy = QPushButton("Копировать результат")
        self.button_copy.clicked.connect(self.on_copy_clicked)
        button_layout.addWidget(self.button_copy)  # Добавляем кнопку в горизонтальный layout
        
        self.button_clear = QPushButton("Очистить")
        self.button_clear.clicked.connect(self.on_clear_clicked)
        button_layout.addWidget(self.button_clear)  # Добавляем кнопку в горизонтальный layout
        
        layout.addLayout(button_layout)  # Добавляем горизонтальный layout в основной вертикальный layout
